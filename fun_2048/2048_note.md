...
---
## 对游戏的一点修改
- 去掉存档功能，每次只有两个选项：开始游戏；退出
- 去掉用户自定义面板大小，统一使用4x4面板
- 增加游戏实时记录用户最高分，并存储在本地文件中，每次开始新的游戏，总是读取这个文件，
显示统计的最高分。
- 原游戏在循环模式中，没有退出选项，只能强制终止。新版考虑增加一个退出选项，用户随时可以退出游戏。
...
### 'std::tie'关键字
- `tie`将变量的引用整合成一个`tuple`，从而实现**批量的赋值**.
```c++
using namespace std;
...
- 只能用在类内部的构造函数声明上，而不能用在类外部的函数定义上。它的作用是不能进行隐式转换。也就是将编译器隐式转换的功能给屏蔽掉。此时这个类的构造函数就不能隐式的创造对象。
